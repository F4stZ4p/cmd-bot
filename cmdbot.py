...
cmdbot = commands.Bot(command_prefix="util.", formatter=None) # prefix (do not change it)

token = '' # paste your bot's token here
whitelist = ["",""]  # add user ids to execute cmd commands

cmdbot.remove_command('help') # removing stock help command
...
async def help(ctx): #main help command
    ...
    elif ctx.message.author.id not in whitelist:
        embed = discord.Embed(title="CMD-Bot", color=0xFF0000) # formatting embed message
        embed.add_field(name="You are not allowed to execute cmd.util command.", value="Only whitelisted users are able to do this.", inline=True) # formatting embed message 2
        await cmdbot.send_message(ctx.message.channel, embed=embed) # sending embed message
...
async def cmd(ctx): # bot command
    if ctx.message.author.id in whitelist: # check message author id before executing cmd command
    ...
    elif ctx.message.author.id not in whitelist: # if somebody is executing CMD-Bot command
        embed = discord.Embed(title="CMD-Bot", color=0xFF0000) # formatting embed message
        embed.add_field(name="You are not allowed to execute cmd.util command.", value="Only whitelisted users are able to do this.", inline=True) # formatting embed message 2
        await cmdbot.send_message(ctx.message.channel, embed=embed) # sending embed message
